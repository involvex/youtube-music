...
app.whenReady().then(async () => {
  ...
  if (is.windows()) {
    const appID = 'com.github.th_ch.pear_music';
...
// Inline implementation of enhanceWebRequest to avoid problematic import
interface BetterSession extends Electron.Session {
  webRequest: any;
}

function enhanceWebRequest(betterSession: BetterSession) {
  // Simple enhancement that just ensures webRequest is available
  // This avoids the problematic dependency chain
...
function removeContentSecurityPolicy(
  ...
  betterSession.webRequest.onHeadersReceived((details: any, callback: any) => {
  ...
  // TODO: Implement proper resolver functionality
  /*
  betterSession.webRequest.setResolver(
    'onHeadersReceived',
    async (listeners: any[]) => {
      return listeners.reduce(
        async (accumulator: any, listener: any) => {
